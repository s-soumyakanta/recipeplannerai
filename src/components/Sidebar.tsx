...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const tabs = [
    ...
    { id: 'availableIngredients', name: 'Available Ingredients', href: '/dashboard/available-ingredients' },
    { id: 'dietPreferences', name: 'Diet Preferences', href: '/dashboard/diet-preferences' },
  ];
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <button
  onClick={() => setIsSidebarOpen(false)}
  className="sm:hidden absolute top-4 right-4 w-8 h-8 flex items-center justify-center focus:outline-none text-red-200 hover:text-white transition-colors duration-200 bg-transparent hover:bg-red-500 rounded-full p-2"
  type="button"
...
>
  X
            ...
            {isSidebarOpen && <h2 className="text-xl font-bold mb-6 text-red-100">RecipePlannerAI</h2>}
            <nav>
              <ul>
                {tabs.map((tab) => (
          ...
          <div className="p-4 bg-red-800 dark:bg-red-900">
            <h3 className="text-sm font-semibold mb-2 text-red-100">Local Weather</h3>
